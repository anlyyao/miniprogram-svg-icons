...
function parseSimplifiedBrandFormat(input: string): IconsCLIOption {
  ...
  while ((match = brandRegex.exec(inner)) !== null) {
    const icons = match[2]
      ...
      .filter(Boolean);
    if (icons.length) result[match[1]] = icons;
  }
...
export function isIconsEmpty(icons: IconsCLIOption): boolean {
  return Array.isArray(icons) ? icons.length === 0 : Object.keys(icons).length === 0;
}
...
export function parseBaseArgs(printHelp: () => void, customArgs: CustomArgHandler[]): BaseCLIOptions {
  ...
  const customArgMap = new Map(customArgs.map((h) => [h.argName, h]));
  ...
  while (i < args.length) {
    ...
    if (arg === '--scan') {
      ...
      i++;
      if (i < args.length) icons = parseIconsArg(args[i++]);
    } else if (arg === '--dry-run') {
      dryRun = true;
      i++;
    } else if (customArgMap.has(arg)) {
      i++;
      if (i < args.length) customArgMap.get(arg)!.setValue(args[i++]);
    } else {
