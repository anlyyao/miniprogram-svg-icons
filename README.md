...
<p align="center">
  支持微信 · 支付宝 · 快手 · 抖音 · 百度 · 小红书 · 京东 | SVG 多色图标 | 按需裁剪 | Monorepo 架构
</p>
...
微信、支付宝、快手、抖音、百度、小红书、京东、等各大平台小程序**普遍不支持或不建议直接内联（inline）`<svg>` 标签**。
...
**本项目提供了一套完整的 SVG 多色图标解决方案**：通过 SVG → Data URI 的转换管道，结合运行时颜色注入机制，实现了在小程序中完美渲染多色 SVG 图标的能力。
...
本仓库支持生成不同平台小程序图标组件，覆盖 **7 大小程序平台**：
...
项目支持 **7 大小程序平台**，分为两类适配模式：

**Behavior 模式**（5 个平台，共用同一套模板逻辑）：
- 微信（`.wxml` / `.wxss`）
- 快手（`.ksml` / `.css`）
- 抖音（`.ttml` / `.ttss`）
- 百度（`.swan` / `.css`）
...
> **说明**：
> - 微信、快手、抖音、百度、京东 5 个平台的组件 API 完全一致，共用同一套模板（仅文件后缀不同）。
